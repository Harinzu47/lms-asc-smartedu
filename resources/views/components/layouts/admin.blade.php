    ...
    <aside class="w-64 bg-[#1a8882] text-white flex-shrink-0 flex flex-col hidden md:flex transition-all duration-300">
        ...
        @auth
            <div class="flex flex-col items-center py-6 border-b border-teal-600/30">
                ...
                <p class="text-xs text-teal-100 mt-1 uppercase tracking-wide">{{ Auth::user()->role->label() ?? 'Admin' }}
                </p>
            </div>
        @endauth
        ...
        <nav class="flex-1 overflow-y-auto py-4 space-y-1 px-3">
            <a href="{{ route('admin.dashboard') }}"
                class="flex items-center px-3 py-2 rounded-lg hover:bg-white/10 transition {{ request()->routeIs('admin.dashboard') ? 'bg-white/20' : '' }}">
            ...
            <a href="{{ route('admin.users') }}"
                class="flex items-center px-3 py-2 rounded-lg hover:bg-white/10 transition {{ request()->routeIs('admin.users') ? 'bg-white/20' : '' }}">
                <svg class="w-5 h-5 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <a href="{{ route('admin.schedule') }}"
                class="flex items-center px-3 py-2 rounded-lg hover:bg-white/10 transition {{ request()->routeIs('admin.schedule') ? 'bg-white/20' : '' }}">
                <svg class="w-5 h-5 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div x-data="{ open: {{ request()->routeIs('admin.master.*') ? 'true' : 'false' }} }">
                <button @click="open = !open"
                    class="w-full flex items-center justify-between px-3 py-2 rounded-lg hover:bg-white/10 transition focus:outline-none">
                ...
                <div x-show="open" class="pl-11 pr-3 space-y-1 mt-1" x-transition>
                    <a href="{{ route('admin.master.kelas') }}"
                        class="block py-2 px-3 rounded hover:bg-white/10 text-sm {{ request()->routeIs('admin.master.kelas') ? 'bg-white/20' : 'text-teal-100' }}">Kelas</a>
                    <a href="{{ route('admin.master.mapel') }}"
                        class="block py-2 px-3 rounded hover:bg-white/10 text-sm {{ request()->routeIs('admin.master.mapel') ? 'bg-white/20' : 'text-teal-100' }}">Mata
                        Pelajaran</a>
                </div>
            </div>
            ...
            <form method="POST" action="{{ route('logout') }}" class="mt-8">
                @csrf
                <button type="submit"
                    class="w-full flex items-center px-3 py-2 rounded-lg hover:bg-red-500/20 text-red-100 hover:text-white transition">
                    <svg class="w-5 h-5 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
