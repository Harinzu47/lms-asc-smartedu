        ...
        <aside class="fixed inset-y-0 left-0 z-30 w-64 bg-[#1a8882] text-white transition-transform duration-300 ease-in-out transform lg:translate-x-0 lg:static lg:inset-0 shadow-xl flex flex-col"
            ...
            <div class="flex flex-col items-center justify-center py-8 border-b border-teal-600/30 bg-[#166e69]">
                <div class="h-20 w-20 rounded-full bg-white/20 flex items-center justify-center p-1 mb-3 shadow-lg">
                    <div class="h-full w-full rounded-full bg-teal-800 flex items-center justify-center border-2 border-white overflow-hidden">
            ...
            <nav class="flex-1 px-4 mt-6 space-y-2 overflow-y-auto">
                {{-- Dashboard --}}
                <a href="{{ route('student.dashboard') }}" class="flex items-center px-4 py-3 text-sm font-medium rounded-lg transition group {{ request()->routeIs('student.dashboard') ? 'bg-teal-800 border-l-4 border-white shadow-lg' : 'hover:bg-teal-700 hover:text-white text-teal-50' }}">
                    <svg class="w-5 h-5 mr-3 {{ request()->routeIs('student.dashboard') ? 'text-white' : 'text-teal-200 group-hover:text-white' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>
                    Dashboard
                </a>

                {{-- Profile --}}
                <a href="{{ route('student.profile') }}" class="flex items-center px-4 py-3 text-sm font-medium rounded-lg transition group {{ request()->routeIs('student.profile') ? 'bg-teal-800 border-l-4 border-white shadow-lg' : 'hover:bg-teal-700 hover:text-white text-teal-50' }}">
                    <svg class="w-5 h-5 mr-3 {{ request()->routeIs('student.profile') ? 'text-white' : 'text-teal-200 group-hover:text-white' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
                    Profile
                </a>

                {{-- Kelas --}}
                <a href="{{ route('student.kelas.index') }}" class="flex items-center px-4 py-3 text-sm font-medium rounded-lg transition group {{ request()->routeIs('student.kelas.*') ? 'bg-teal-800 border-l-4 border-white shadow-lg' : 'hover:bg-teal-700 hover:text-white text-teal-50' }}">
                    <svg class="w-5 h-5 mr-3 {{ request()->routeIs('student.kelas.*') ? 'text-white' : 'text-teal-200 group-hover:text-white' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" /></svg>
                    Kelas Saya
                </a>

                 {{-- Pembayaran --}}
                <a href="{{ route('student.pembayaran') }}" class="flex items-center px-4 py-3 text-sm font-medium rounded-lg transition group {{ request()->routeIs('student.pembayaran') ? 'bg-teal-800 border-l-4 border-white shadow-lg' : 'hover:bg-teal-700 hover:text-white text-teal-50' }}">
                    <svg class="w-5 h-5 mr-3 {{ request()->routeIs('student.pembayaran') ? 'text-white' : 'text-teal-200 group-hover:text-white' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" /></svg>
                    Pembayaran
                </a>
            </nav>
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <header class="bg-white shadow-sm z-20 h-20 flex items-center justify-between px-6 lg:px-10">
                <div class="flex items-center">
                    <button @click="sidebarOpen = !sidebarOpen" class="text-gray-500 hover:text-teal-600 focus:outline-none mr-4 lg:hidden">
