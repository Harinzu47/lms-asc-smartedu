                        ...
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                @if(!$user->status_aktif && $user->role === \App\Enums\UserRole::SISWA && $user->bukti_pembayaran)
                                    <button @click="$wire.call('openVerification', {{ $user->id }})" type="button" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 hover:bg-yellow-200 cursor-pointer animate-pulse">
                                        Perlu Verifikasi
                                    </button>
                                @else
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end space-x-2">
                                    @if($user->bukti_pembayaran)
                                        <button @click="$wire.call('openVerification', {{ $user->id }})" type="button" class="inline-flex items-center justify-center p-2 rounded-lg bg-blue-100 text-blue-600 hover:bg-blue-200 hover:text-blue-800 transition-colors duration-200 cursor-pointer" title="Lihat Bukti Bayar">
                                            <svg class="w-5 h-5 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                                        </button>
                                    @endif
                                    <button wire:click.prevent="confirmResetPassword({{ $user->id }})" type="button" class="inline-flex items-center justify-center p-2 rounded-lg bg-yellow-100 text-yellow-600 hover:bg-yellow-200 hover:text-yellow-800 transition-colors duration-200" title="Reset Password">
                                    ...
                                    <button wire:click.prevent="edit({{ $user->id }})" type="button" class="inline-flex items-center justify-center p-2 rounded-lg bg-indigo-100 text-indigo-600 hover:bg-indigo-200 hover:text-indigo-800 transition-colors duration-200" title="Edit">
                                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                    </button>
                                    <button wire:click.prevent="confirmDelete({{ $user->id }})" type="button" class="inline-flex items-center justify-center p-2 rounded-lg bg-red-100 text-red-600 hover:bg-red-200 hover:text-red-800 transition-colors duration-200" title="Hapus">
