<div class="space-y-6">
    ...
    <div wire:key="discussion-{{ $discussion->id }}" class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <!-- Header -->
        <div class="p-6 pb-0 flex items-start space-x-4">
            <div class="flex-shrink-0">
                @if(optional($discussion->author)->profile_photo_path)
                    <img class="h-10 w-10 rounded-full object-cover" src="{{ Storage::url($discussion->author->profile_photo_path) }}" alt="{{ optional($discussion->author)->name }}">
                @else
                    <div class="h-10 w-10 rounded-full bg-gray-200 flex items-center justify-center text-gray-500 font-bold">
                        {{ substr(optional($discussion->author)->name ?? '?', 0, 1) }}
                    </div>
                @endif
            </div>
            <div class="flex-1 min-w-0">
                <p class="text-sm font-medium text-gray-900">
                    <span class="font-bold text-blue-900">{{ optional($discussion->author)->name ?? 'Unknown User' }}</span>
                    <span class="text-gray-500 font-normal">memposting informasi</span>
                    <span class="text-gray-400 mx-1">&gt;</span>
                    <span class="font-bold text-teal-600">{{ optional(optional($discussion->jadwal)->mapel)->nama_mapel ?? 'Mapel' }}</span>
                </p>
                <p class="text-xs text-gray-500">
                    {{ $discussion->created_at ? $discussion->created_at->diffForHumans() : '' }}
                </p>
            </div>
        </div>
        ...
        <div class="bg-gray-50 px-6 py-4 border-t border-gray-100">
            ...
            <div class="space-y-3 mb-4 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                ...
                <div wire:key="comment-{{ $comment->id }}" class="flex space-x-3">
                    <div class="flex-shrink-0">
                         @if(optional($comment->author)->profile_photo_path)
                        ...
                        @else
                            <div class="h-8 w-8 rounded-full bg-gray-200 flex items-center justify-center text-xs text-gray-500 font-bold">
                                {{ substr(optional($comment->author)->name ?? '?', 0, 1) }}
                            </div>
                        @endif
                    </div>
                    <div class="flex-1 bg-white p-3 rounded-lg shadow-sm border border-gray-100">
                        <div class="flex justify-between items-start">
                            <span class="text-sm font-bold text-gray-800">{{ optional($comment->author)->name ?? 'Unknown' }}</span>
