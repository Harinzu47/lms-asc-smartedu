<div>
    @if(!$isConnected)
        {{-- Join Session Form --}}
        <div
            class="min-h-screen flex items-center justify-center bg-gradient-to-br from-purple-50 to-teal-50 -m-6 lg:-m-10">
            <div class="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full mx-4">
                <div class="text-center mb-8">
                    <div
                        class="w-20 h-20 bg-gradient-to-br from-purple-500 to-teal-500 rounded-2xl flex items-center justify-center mx-auto mb-4">
    ...
    @else
        ...
        <div wire:ignore id="whiteboard-container" class="min-h-screen flex flex-col -m-6 lg:-m-10">
            ...
            {{-- Toolbar --}}
            <div
                class="bg-gradient-to-r from-gray-100 to-gray-50 px-4 py-3 flex flex-wrap items-center justify-center gap-4 border-b border-gray-200 shadow-sm">
                {{-- Color Picker --}}
                <div class="flex items-center gap-2 bg-white rounded-xl px-4 py-2 shadow-sm border border-gray-200">
                    ...
                    <button id="color-red"
                        class="w-8 h-8 bg-red-500 rounded-full hover:scale-110 transition transform shadow-md border-2 border-white"></button>
                    <button id="color-blue"
                        class="w-8 h-8 bg-blue-500 rounded-full hover:scale-110 transition transform shadow-md border-2 border-white"></button>
                    <button id="color-black"
                        class="w-8 h-8 bg-gray-800 rounded-full hover:scale-110 transition transform shadow-md border-2 border-white ring-2 ring-offset-2 ring-gray-800"></button>
                    <button id="color-green"
                        class="w-8 h-8 bg-green-500 rounded-full hover:scale-110 transition transform shadow-md border-2 border-white"></button>
                </div>
            ...
            (function () {
                ...
                function init() {
                    ...
                    canvas.on('path:created', function (e) {
                        // Compress path data to reduce payload size
                        const pathData = compressPathData(e.path.toJSON());
                ...
                // Compress path data to reduce payload size (Pusher has 10KB limit)
                function compressPathData(pathObj) {
                    // Reduce coordinate precision to 1 decimal place
                    if (pathObj.path && Array.isArray(pathObj.path)) {
                        pathObj.path = pathObj.path.map(segment => {
                            return segment.map(val => {
                    ...
                    // Round other numeric properties
                    ['left', 'top', 'width', 'height', 'scaleX', 'scaleY'].forEach(prop => {
                        if (typeof pathObj[prop] === 'number') {
                            pathObj[prop] = Math.round(pathObj[prop] * 10) / 10;
                        }
                    });

                    // Remove unnecessary properties
                    delete pathObj.version;
                    delete pathObj.originX;
                    delete pathObj.originY;
                    delete pathObj.angle;
                    delete pathObj.flipX;
                    delete pathObj.flipY;
                    delete pathObj.opacity;
                    delete pathObj.shadow;
                    delete pathObj.visible;
                    delete pathObj.backgroundColor;
                    delete pathObj.fillRule;
                    delete pathObj.paintFirst;
                    delete pathObj.globalCompositeOperation;
                    delete pathObj.skewX;
                    delete pathObj.skewY;
